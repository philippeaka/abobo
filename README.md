...
- `packages/@abobo/core`: Core React components
- `packages/@abobo/utils`: Shared React utilities
- `packages/@abobo/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset-abobo`: ESLint preset
...
For `abobo-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `abobo-core` then instructs the consumer to select the correct format:

```json:abobo-core/package.json
{
  "name": "@abobo/core",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `abobo-core/dist` which contains the compiled output.
...
Each file inside of `abobo-core/src` is a component inside our design system. For example:

```tsx:abobo-core/src/Button.tsx
...
- Support using module path aliases like `@abobo-core` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@abobo-core/src';
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `abobo` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `abobo` with your desired scope
- Search and replace `abobo` with your desired scope
- Re-run `yarn install`
